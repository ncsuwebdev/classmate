// This has been modified slightly by Garrison Locke to add the Event ID to the 
// id attr of the event so it can easily be selected by jQuery (line 268 in
// the DrawEventsOnCalendar function)
		...
		if (calendarEvents && calendarEvents.length > 0) {
			jQuery.each(calendarEvents, function(){
				...
				if ((ev.Date >= _beginDate) && (ev.Date <= _endDate)) {
					...
					// added by Garrison Locke
					event.attr('id', 'Event_' + ev.EventID);
	...
	jQuery.J.AddEvents = function(eventCollection) {
		if(eventCollection) {
			if(eventCollection.length > 1) {
				...
				//add new single event to end of array
